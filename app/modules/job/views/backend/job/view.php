					...
					<div class="ibox-title">
						...
						<div class="ibox-tools">
							...
							<ul class="dropdown-menu dropdown-job">
								...
								<li><a type="button" class="ajax-delete-all" data-title="Lưu ý: Khi bạn thực hiện thao tác này, toàn bộ dữ liệu sẽ không thể khôi phục được. Hãy chắc chắn rằng bạn muốn thực hiện chức năng này!" data-module="job">Xóa tất cả</a>
						...
						<div class="table-responsive">
							...
							<div class="uk-flex uk-flex-middle uk-flex-space-between">
								...
								<div class="perpage">
									...
									<div class="uk-flex uk-flex-middle mb10 hidden">
										...
										<?php echo form_dropdown('perpage', $this->configbie->data('perpage'), set_value('perpage',$this->input->get('perpage')) ,'class="form-control input-sm perpage filter"  data-url="'.site_url('job/backend/job/view').'"'); ?>
								...
								<div class="toolbox">
									...
									<div class="uk-flex uk-flex-middle uk-flex-space-between">
										...
										<div class="uk-button">
											...
											<a href="<?php echo site_url('job/backend/job/create'); ?>" class="btn btn-danger btn-sm"><i class="fa fa-plus"></i> Thêm bài viết mới</a>
							...
							<table class="table table-striped table-bordered table-hover dataTables-example" >
								...
								<tbody id="ajax-content">
									...
									<?php if(isset($listjob) && is_array($listjob) && count($listjob)){ ?>

										<?php foreach($listjob as $key => $val){ ?>
										...
										?>
											...
											<tr class="gradeX" id="post-<?php echo $val['id']; ?>">
												...
												<td>
													...
													<div class="uk-flex uk-flex-middle">
														...
														<div class="main-info">
															...
															<div class="title"><a class="maintitle" href="<?php echo site_url('job/backend/job/update/'.$val['id']); ?>" title=""><?php echo $val['title']; ?></a>
															...
															<div class="catalogue" style="font-size:10px">
																...
																<a class="" style="color:#333;" href="<?php echo site_url('job/backend/job/view?cataloguesid='.$val['catalogueid'].'')?>" title=""><?php echo $val['catalogue_title']; ?></a><?php echo (isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list) ) ? ' ,' :''; ?>

																<?php if(isset($_catalogue_list) && is_array($_catalogue_list) && count($_catalogue_list)){ foreach($_catalogue_list as $keyCat => $valCat){ ?>

																<a style="color:#333;" class="" href="<?php echo site_url('job/backend/job/view?catalogueid='.$valCat['id']); ?>" title=""><?php echo $valCat['title']; ?></a> <?php echo ($keyCat + 1 < count($_catalogue_list)) ? ', ' : ''; ?>
												...
												<td>
													...
													<?php echo form_input('order['.$val['id'].']', $val['order'], 'data-module="job" data-id="'.$val['id'].'"  class="form-control sort-order" placeholder="Vị trí" style="width:50px;text-align:right;"');?>
												...
												<td class="text-center">
													...
													<a type="button" href="<?php echo site_url('job/backend/job/update/'.$val['id'].'') ?>" class="btn btn-primary" title="Thêm"><i class="fa fa-edit"></i></a>

													<a type="button" class="btn btn-danger ajax-delete" data-title="Lưu ý: Dữ liệu sẽ không thể khôi phục. Hãy chắc chắn rằng bạn muốn thực hiện hành động này!" data-router="<?php echo $val['canonical']; ?>" data-id="<?php echo $val['id'] ?>" data-module="job" title="Sửa"><i class="fa fa-trash"></i></a>
													<a type="button" href="<?php echo site_url('job/backend/job/duplicate/'.$val['id'].'?page=1') ?>" class="btn btn-info" title="Sao chép"><i class="fa fa-files-o" aria-hidden="true"></i></a>
													<a target="_blank" href="<?php echo $val['canonical'] ?>.html" class="btn btn-info hidden" title="Xem trước"><i class="fa fa-eye" aria-hidden="true"></i></a>
