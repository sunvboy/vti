    ...
    public function listCatalogue()
    {
        ...
        if ($config['total_rows'] > 0) {
            ...
            $listCatalogue = $this->Autoload_Model->_get_where(array(
                'select' => 'id, title, publish, created, order, level,ishome,highlight,isaside,isfooter, (SELECT fullname FROM user WHERE user.id = media_catalogue.userid_created) as user_created',
                'table' => 'media_catalogue',
                'where' => array('alanguage' => $this->fclang),
                'keyword' => '(title LIKE \'%' . $keyword . '%\' OR description LIKE \'%' . $keyword . '%\')',
                        ...
                        $html = $html . '<td class="">
													<div class="switch">
														<div class="onoffswitch">
															<input type="checkbox" ' . (($val['' . $valIS['is'] . ''] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="media_catalogue" data-title="' . $valIS['is'] . '" data-id="' . $val['id'] . '" id="publish_' . $valIS['is'] . '-' . $val['id'] . '">
