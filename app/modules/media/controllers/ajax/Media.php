        ...
        if ($config['total_rows'] > 0) {
            ...
            if ($catalogueid > 0) {
                $listCatalogue = $this->Autoload_Model->_condition(array(
                    'module' => 'media',
                    'select' => '`object`.`id`, `object`.`title`,`object`.`ishome`,`object`.`highlight`,`object`.`isfooter`,`object`.`isaside`, `object`.`slug`,`object`.`canonical`, `object`.`catalogueid`, `object`.`catalogue`, `object`.`publish`, `object`.`image`, `object`.`image_json`, `object`.`created`, `object`.`order`, `object`.`viewed`, (SELECT fullname FROM user WHERE user.id = object.userid_created) as user_created, (SELECT title FROM media_catalogue WHERE media_catalogue.id = object.catalogueid) as catalogue_title',
                    'where' => ((!empty($extend)) ? '`object`.`userid_created` = ' . $this->auth['id'] . ' AND `object`.`alanguage` = \'' . $this->fclang . '\'' : ' `object`.`alanguage` = \'' . $this->fclang . '\''),
                    'keyword' => '(`object`.`title` LIKE \'%' . $keyword . '%\' AND `object`.`description` LIKE \'%' . $keyword . '%\')',
                    'catalogueid' => $catalogueid,
                    'limit' => $perpage,
                ...
                $listCatalogue = $this->Autoload_Model->_get_where(array(
                    'select' => 'id, catalogueid, catalogue, title, canonical, publish,ishome,highlight,isfooter,isaside, created, order, viewed, image, image_json, (SELECT fullname FROM user WHERE user.id = media.userid_created) as user_created, (SELECT title FROM media_catalogue WHERE media_catalogue.id = media.catalogueid) as catalogue_title',
                    'table' => 'media',
                    'where_extend' => $extend,
                    'where' => array('alanguage' => $this->fclang),
                        ...
                        $html = $html . '<td class="">
													<div class="switch">
														<div class="onoffswitch">
															<input type="checkbox" ' . (($val['' . $valIS['is'] . ''] == 1) ? 'checked=""' : '') . ' class="onoffswitch-checkbox publish_frontend" data-module="media" data-title="' . $valIS['is'] . '" data-id="' . $val['id'] . '" id="publish_' . $valIS['is'] . '-' . $val['id'] . '">
