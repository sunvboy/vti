		...
		if($config['total_rows'] > 0){
			...
			if(isset($listSupport) && is_array($listSupport) && count($listSupport)){
				foreach($listSupport as $key => $val){
					...
					$html = $html .'<p><strong class="text-blue">Số điện thoại:</strong> '.$val['phone'].'</p>';
                    $html = $html .'<p><strong class="text-blue">Hotline:</strong> '.$val['zalo'].'</p>';
                    $html = $html .'<p><strong class="text-blue">Email:</strong> '.$val['email'].'</p>';
					$html = $html .'<p><strong class="text-blue">Địa chỉ:</strong> '.$val['skype'].'</p>';
