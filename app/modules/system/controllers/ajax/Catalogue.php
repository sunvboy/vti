    ...
    public function listCatalogue()
    {
        ...
        if (isset($listCatalogue) && is_array($listCatalogue) && count($listCatalogue)) {
            foreach ($listCatalogue as $key => $val) {
                ...
                $html = $html . '<td><a class="maintitle" style="' . (($val['level'] == 1) ? 'font-weight:600;' : '') . '" href="' . site_url('systems/backend/systems/view?cataloguesid=' . $val['id'] . '') . '" title="">' . (str_repeat('|----------', (($val['level'] > 0) ? ($val['level'] - 1) : 0)) . $val['title']) . ' (0)</a></td>';
                $html = $html . '<td>';
                ...
                $html = $html . '<a type="button" href="' . (site_url('system/backend/catalogue/update/' . $val['id'] . '')) . '" class="btn btn-sm btn-primary mr5"><i class="fa fa-edit"></i></a>';
                $html = $html . '<a type="button" class="btn btn-sm btn-danger ajax-delete" data-title="Lưu ý: Khi bạn xóa danh mục, toàn bộ bài viết trong nhóm này sẽ bị xóa. Hãy chắc chắn bạn muốn thực hiện chức năng này!" data-id="' . $val['id'] . '" data-module="system_catalogue"><i class="fa fa-trash"></i></a>';
                $html = $html . '</td>';
                $html = $html . '</tr>';
            }
        } else {
